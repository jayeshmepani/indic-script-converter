...
function toLosslessDevanagariFromGujarati(
    string $input,
    ?IndicScriptConversionOptions $options = null,
): LosslessTransliterationResult {
    $options ??= new IndicScriptConversionOptions;
    $visibleInput = stripExactSourceMetadata($input);
    $normalizedInput = normalizeUnicode($visibleInput, $options->inputNormalization);
    return new LosslessTransliterationResult(
        original: $input,
        normalizedInput: $normalizedInput,
        rendered: toCanonicalDevanagariFromGujarati($visibleInput, $options),
        ...
        issues: [
            new TransliterationIssue(
                ...
                message: 'Gujarati and Devanagari have unequal repertoires. Keep this envelope or enable exact-source metadata for exact recovery.',
...
function to_lossless_devanagari_from_gujarati(string $input, ?IndicScriptConversionOptions $options = null): LosslessTransliterationResult
{
    return toLosslessDevanagariFromGujarati($input, $options);
}

function toLosslessGujaratiFromDevanagari(
    string $input,
    ?IndicScriptConversionOptions $options = null,
): LosslessTransliterationResult {
    $options ??= new IndicScriptConversionOptions;
    ...
    return new LosslessTransliterationResult(
        original: $input,
        normalizedInput: $normalizedInput,
        rendered: toCanonicalGujaratiFromDevanagari($visibleInput, $options),
        profile: TransliterationProfile::EXTENDED_INDIC,
        inputNormalization: $options->inputNormalization,
        outputNormalization: $options->outputNormalization,
        renderingIsInjective: false,
        issues: [
            new TransliterationIssue(
                code: 'SOURCE_METADATA_REQUIRED_FOR_EXACT_SCRIPT_REVERSE',
                message: 'Devanagari and Gujarati have unequal repertoires. Keep this envelope or enable exact-source metadata for exact recovery.',
                severity: TransliterationIssueSeverity::INFO,
            ),
        ],
    );
...
function to_lossless_gujarati_from_devanagari(string $input, ?IndicScriptConversionOptions $options = null): LosslessTransliterationResult
{
    return toLosslessGujaratiFromDevanagari($input, $options);
}
